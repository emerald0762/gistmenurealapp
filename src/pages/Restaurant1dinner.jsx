...
export default function Restaurant1Dinner() {
  ...
  return (
    <Stack direction="row" spacing={20} sx={{justifyContent: "space-evenly", alignItems: "center"}} >
     <Button variant="text" onClick={() => navigate('/restaurant1lunch')} sx={{color: '#000000', fontSize:'3rem'}}>점심</Button>
     <Stack spacing={1} alignItems="column" sx={{justifyContent: "space-evenly", alignItems: "center"}}>
       <Typography variant="h3" sx={{ color: '#000000' }}>{todayDisplay} 저녁</Typography>
        {menu.length > 0 ? (
          menu.map((item, index) => (
